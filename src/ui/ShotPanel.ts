...
export class ShotPanel {
  ...
  private createPanel(): void {
    ...
    // Title
    addUIElement(this.scene.add.text(panelX + panelW / 2, panelY + 10, t('shotPanel.title'), {
      fontSize: '14px',
      color: '#ffffff',
      fontStyle: 'bold',
    ...
    for (let i = 0; i < CLUBS.length; i++) {
      const club = CLUBS[i];
      const btn = addUIElement(this.scene.add.text(
      ...
      ).setOrigin(0.5, 0).setInteractive({ useHandCursor: true }), this.elements, 2000);
    ...
    addUIElement(this.scene.add.text(panelX + panelW / 2, spinY, t('shotPanel.spin'), {
    ...
    for (let i = 0; i < spinOptions.length; i++) {
      ...
      const sBg = addUIElement(this.scene.add.graphics(), this.elements, 2000);
      this.spinBgs.push(sBg);

      // Text
      const sText = addUIElement(this.scene.add.text(
        btnX + spinBtnW / 2, spinBtnY + 14, opt.label,
        { fontSize: '20px', color: '#aaaaaa' }
      ).setOrigin(0.5, 0.5), this.elements, 2000);
      this.spinTexts.push(sText);

      // Invisible clickable rectangle (NOT in a container)
      const hitArea = addUIElement(
